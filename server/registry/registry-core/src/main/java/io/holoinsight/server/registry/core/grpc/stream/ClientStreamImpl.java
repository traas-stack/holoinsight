...
public class ClientStreamImpl implements ClientStream {
    ...
    private static final AtomicIntegerFieldUpdater<ClientStreamImpl> U = AtomicIntegerFieldUpdater.newUpdater(ClientStreamImpl.class, "status");
    ...
    private class Reader implements StreamObserver<GenericRpcCommand> {
        ...
        public void onNext(GenericRpcCommand resp) {
            try {
                if (needFirst) {
                    needFirst = false;
                    if (U.compareAndSet(ClientStreamImpl.this, STATUS_HAND_SHAKE_SENT, STATUS_HAND_SHAKE_RECVED)) {
                        onHandshakeCmd(resp);
                    }
                    return;
                }
