...
public interface TraceApi {
    ...
    @ApiOperation(
            value = "query basic traces",
            nickname = "queryTraces",
            notes = "根据查询条件查询一批trace的基本信息。",
            response = TraceBrief.class,
            authorizations = {
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "请求正常。", response = TraceBrief.class),
    ...
    ResponseEntity<TraceBrief> queryBasicTraces(
            @ApiParam(value = "trace查询条件。", required = true) @Valid @RequestBody
            QueryTraceRequest request) throws IOException;
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "请求正常。", response = Trace.class),
                    @ApiResponse(code = 400, message = "请求失败。", response = FailResponse.class)
            })
    @RequestMapping(
            value = "/query",
            ...
            consumes = {"application/json"},
            method = RequestMethod.POST)
    ResponseEntity<Trace> queryTrace(
            @ApiParam(value = "trace查询条件。", required = false) @Valid @RequestBody
            QueryTraceRequest request) throws IOException;
}