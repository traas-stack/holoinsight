...
public class TraceAgentFacadeImpl extends BaseFacade {
  ...
  @Value("${traceAuthEncrypt.enable:false}")
  private boolean traceAuthEncryptEnable;

  @Value("${traceAuthEncrypt.secretKey}")
  private String secretKey;

  @Value("${traceAuthEncrypt.iv}")
  private String iv;
  ...
  public JsonResult<Map<String, Object>> traceAgent(
    ...
    try {
      ...
      if (!CollectionUtils.isEmpty(extendInfo)) {
        ...
        if (traceAuthEncryptEnable && !StringUtils.isEmpty(secretKey)) {
          apikey = AesUtil.aesEncrypt(apikey, secretKey, iv);
        }
      }
