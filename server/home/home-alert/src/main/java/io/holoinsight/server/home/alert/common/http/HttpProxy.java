...
public class HttpProxy {
    ...
    public static void requestWithRetry (XHttpRequest request, HttpAsync.HttpAsyncRetryHandler handler, int retryCnt, int maxRetryTimes) {
        if (httpAsyncClient == null) {
            start();
        }
        // 确保header中有请求发送时间
    ...
    public static XHttpResponse requestWithRetry(XHttpRequest request, int retryCnt, int maxRetryTimes) throws Exception {
        XHttpResponse response;
        try {
            ...
            if (retryCnt >= maxRetryTimes) {
                throw new RuntimeException(
                        String.format("request fail and reach the max retry times[%s], %s. ", maxRetryTimes, e.getMessage()), e);
            } else {
    ...
    public static XHttpResponse request(XHttpRequest request) throws Exception {
        ...
        latch.await();
        if (optional.sucess) {
            logger.info("http request[" + request + "] cost " + (System.currentTimeMillis() - start) + "ms, code["
                    + optional.response.code + "].");
            return optional.response;
        } else {
            logger.error("http request[" + request + "] fail, error: " + optional.error, optional.exception);
