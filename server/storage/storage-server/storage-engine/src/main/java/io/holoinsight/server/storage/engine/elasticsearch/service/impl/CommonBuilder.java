...
public class CommonBuilder {
    ...
    public static void addTermParamsWithAttr(BoolQueryBuilder queryBuilder, Map<String, String> termParams) {
        if (termParams != null && termParams.size() > 0) {
            termParams.keySet().forEach(k -> {
    ...
    public static TermsAggregationBuilder buildAgg(String aggField) {
        TermsAggregationBuilder aggregationBuilder = AggregationBuilders
                .terms(aggField).field(aggField)
                .subAggregation(
                        AggregationBuilders.terms(ServiceRelationEsDO.COMPONENT)
                                .field(ServiceRelationEsDO.COMPONENT)
                                .executionHint("map")
                                .collectMode(Aggregator.SubAggCollectionMode.BREADTH_FIRST))
                .subAggregation(AggregationBuilders.avg("avg_latency").field(ServiceRelationEsDO.LATENCY))
                .subAggregation(AggregationBuilders.percentiles("p95_latency").field(ServiceRelationEsDO.LATENCY).percentiles(95.0))
                .subAggregation(AggregationBuilders.percentiles("p99_latency").field(ServiceRelationEsDO.LATENCY).percentiles(99.0))
                .subAggregation(AggregationBuilders.count("total_count").field(ServiceRelationEsDO.TRACE_ID))
                .subAggregation(AggregationBuilders.filter(ServiceRelationEsDO.TRACE_STATUS, QueryBuilders.termQuery(ServiceRelationEsDO.TRACE_STATUS, Status.StatusCode.STATUS_CODE_ERROR_VALUE))
    ...
    public static ResponseMetric buildMetric(Terms.Bucket bucket) {
        ...
        ResponseMetric metric = new ResponseMetric(latency, p95Latency, p99Latency, totalCount, errorCount);
