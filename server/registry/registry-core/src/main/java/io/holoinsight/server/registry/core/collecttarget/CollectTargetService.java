...
public class CollectTargetService {
    ...
    public String buildRefAgent(CollectTemplate t, Target dimRow) {
        ...
        if (refAgent == null) {
            String msg = String.format("table=[%d/%s] dim=[%s]", t.getId(), t.getTableName(), dimRow.getId());
        ...
        switch (es.getType()) {
            ...
            case ExecutorSelector.CENTRAL:
                ...
                if (cluster.getAgents().isEmpty()) {
                    throw new IllegalStateException("central agent cluster " + requiredClusterName + " is empty");
                }
                // TODO 使用一致性哈希
                Agent agent = cluster.getRing().select(t.getTableName().hashCode());
                if (agent == null) {
                    throw new IllegalStateException("central agent cluster " + requiredClusterName + " hash ring select null");
                }
                return agent.getId();
            default:
                throw new IllegalArgumentException("unsupported ExecutorSelector " + es.getType());
