...
public class GroovyWebController {
    ...
    @PostMapping("/execute")
    public Object execute(@RequestHeader("token") String token, @RequestBody String body, HttpServletRequest request) {
        String remoteAddr = request.getRemoteAddr();
        if (!"127.0.0.1".equals(remoteAddr) && !"0:0:0:0:0:0:0:1".equals(remoteAddr)) {
            return "forbidden";
        }
        if (StringUtils.isEmpty(config.getBasic().getToken()) || !token.equals(config.getBasic().getToken())) {
            return "forbidden";
        }
