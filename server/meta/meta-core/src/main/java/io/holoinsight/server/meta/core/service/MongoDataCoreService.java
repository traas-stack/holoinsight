...
public class MongoDataCoreService extends AbstractDataCoreService {
    ...
    public io.holoinsight.server.common.Pair<Integer, Integer> insertOrUpdate(String tableName,
        ...
        if(CollectionUtils.isEmpty(rows)) return new io.holoinsight.server.common.Pair<>(0, 0);
        ...
        logger.info("[insertOrUpdate] finish, table={}, upsertSize={}, matchedCount={}, modifiedCount={}, cost={}.", tableName, upsertSize,
                matchedCount, modifiedCount, stopWatch.getTime());
    ...
    public List<Map<String, Object>> update(String tableName, List<Map<String, Object>> rows) {
        ...
        rows.forEach(row -> {
            if (row.containsKey(default_pk)) {return;}
            Document document = mongoDbHelper.modifyById(row.get(default_pk).toString(), row, Document.class, tableName);
            documents.add(document);
            logger.info("[update] finish, table={}, record={}, cost={}.", tableName, document.get(default_pk), stopWatch.getTime());
        });
    ...
    @Override
    public List<Map<String, Object>> updateByExample(String tableName, QueryExample queryExample, Map<String, Object> row) {
        logger.info("[updateByExample] finish, table={}, queryExample={}, row={}.", tableName, J.toJson(queryExample),
                J.toJson(row));
        StopWatch stopWatch = StopWatch.createStarted();
        ...
        list.forEach(l -> {
            l.putAll(row);
            Document document = mongoDbHelper.modifyById(l.get(default_pk).toString(), l, Document.class, tableName);
            documents.add(document);
            logger.info("[update] finish, table={}, record={}, cost={}.", tableName, document.get(default_pk), stopWatch.getTime());
    ...
    public List<Map<String, Object>> queryByPks(String tableName, List<String> pkValList) {
        ...
        if(CollectionUtils.isEmpty(pkValList)) return new ArrayList<>();
    ...
    public List<Map<String, Object>> queryByExample(String tableName, QueryExample queryExample) {
        logger.info("[queryByExample] finish, table={}, queryExample={}.", tableName, J.toJson(queryExample));
        StopWatch stopWatch = StopWatch.createStarted();
        Query query = new Query();
        query.with(Sort.by(Direction.DESC, default_modified));
        if (!CollectionUtils.isEmpty(queryExample.getParams())) {
            for (Map.Entry<String, Object> entry : queryExample.getParams().entrySet()) {
                ...
                if (entry.getValue() instanceof String) {
                    ...
                    List<String> o = J.fromJson(J.toJson(entry.getValue()), new TypeToken<List<String>>() {
                    }.getType());
    ...
    public List<Map<String, Object>> fuzzyByExample(String tableName, QueryExample queryExample) {
        logger.info("[fuzzyByExample] finish, table={}, queryExample={}.", tableName, J.toJson(queryExample));
        StopWatch stopWatch = StopWatch.createStarted();
        Query query = new Query();
        query.with(Sort.by(Direction.DESC, default_modified));
        ...
        if (!CollectionUtils.isEmpty(queryExample.getParams())) {
            for (Map.Entry<String, Object> entry : queryExample.getParams().entrySet()) {
                ...
                orCriteria.add(Criteria.where(entry.getKey()).regex(J.json2Bean(J.toJson(entry.getValue()), Pattern.class)));
            }
        }
    ...
    //    logger.info("[queryPksByExample] finish, table={}, queryExample={}.", tableName, J.toJson(queryExample));
    //    StopWatch stopWatch = StopWatch.createStarted();
    //    List<Map<String, Object>> list = queryByExample(tableName, queryExample);
    //    if (CollectionUtils.isEmpty(list)) { return new ArrayList<>(); }
    //
    //    Collection<String> rows = Collections2.transform(list, new Function<Map<String, Object>, String>() {
    //        @Override
    //        public String apply(Map<String, Object> input) {
    //            return input.get(default_pk).toString();
    //        }
    //    });
    //    logger.info("[queryPksByExample] finish, table={}, records={}, cost={}.", tableName, rows.size(),
    //            stopWatch.getTime());
    //
    ...
    public long deleteByExample(String tableName, QueryExample queryExample) {
        logger.info("[deleteByExample] finish, table={}, queryExample={}.", tableName, J.toJson(queryExample));
        ...
        // 输出结果信息
        logger.info("[deleteByExample] finish, table={}, deleteCount={}, cost={}.", tableName, deleteCount, stopWatch.getTime());
        return deleteCount;
    }
    ...
    public long batchDeleteByPk(String tableName, List<String> default_pks) {
        logger.info("[batchDeleteByPk] finish, table={}, default_pks={}.", tableName, default_pks.size());

        if(CollectionUtils.isEmpty(default_pks)) return 0;
        StopWatch stopWatch = StopWatch.createStarted();
        Query query = new Query();
        ...
        long deleteCount = mongoDbHelper.removeByCondition(query, tableName);
        logger.info("[batchDeleteByPk] finish, table={}, deleteCount={}, cost={}.", tableName, deleteCount, stopWatch.getTime());
        return deleteCount;
    }
}