...
public class CustomPluginUpdateListener {
    ...
    private List<Long> upsert(Map<String, SqlTask> sqlTasks, CustomPluginDTO customPluginDTO) {
        ...
        List<GaeaCollectConfigDTO> byRefId = gaeaCollectConfigService
            .findByRefId("custom_" + customPluginDTO.getId());
    ...
    private SqlTask buildSqlTask(List<LogPath> logPaths, CollectMetric collectMetric,
        ...
        Map<String, Map<String, SplitCol>> splitColMap = GaeaSqlTaskUtil.convertSplitColMap(conf.splitCols);
        Select select = GaeaSqlTaskUtil.buildSelect(conf.logParse, splitColMap, collectMetric);
        From from = GaeaSqlTaskUtil.buildFrom(logPaths, conf.logParse, conf.whiteFilters, conf.blackFilters);
        ...
        Output output = GaeaSqlTaskUtil.buildOutput(getTargetTableName(customPluginDTO.id, collectMetric));
