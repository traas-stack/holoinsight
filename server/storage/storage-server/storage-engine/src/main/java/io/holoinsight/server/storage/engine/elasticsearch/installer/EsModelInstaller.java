...
public class EsModelInstaller implements ModelInstaller {
    ...
    public boolean isExists(Model model) {
        ...
        try {
            boolean isExists = indicesClient.existsIndexTemplate(new ComposableIndexTemplateExistRequest(tableName),
            ...
            if (isExists && indexTemplatesResponse != null && indexTemplatesResponse.getIndexTemplates().containsKey(tableName)) {
                ComposableIndexTemplate composableIndexTemplate = indexTemplatesResponse.getIndexTemplates().get(tableName);
    ...
    private void createTimeSeriesTable(Model model) {
        ...
        try {
            ...
            shouldUpdateTemplate = shouldUpdateTemplate || !structures.containsStructure(tableName, mappings);
            if (shouldUpdateTemplate) {
                structures.putStructure(tableName, mappings);
                Settings settings = createSettings(model);
                CompressedXContent mappingCompressedXContent = new CompressedXContent(EsGsonUtils.esGson().toJson(mappings));
                PutComposableIndexTemplateRequest putComposableIndexTemplateRequest = new PutComposableIndexTemplateRequest().name(
                        tableName);
                ...
                ComposableIndexTemplate composableIndexTemplate = new ComposableIndexTemplate(Arrays.asList(tableName + "-*"), template,
                        null, null,
                        null, null);
                putComposableIndexTemplateRequest.indexTemplate(composableIndexTemplate);
                AcknowledgedResponse acknowledgedResponse = indicesClient.putIndexTemplate(putComposableIndexTemplateRequest,
                        RequestOptions.DEFAULT);
            ...
            String indexName = tableName + Const.LINE + TimeBucket.getTimeBucket(System.currentTimeMillis(), DownSampling.Day);
    ...
    protected Settings createSettings(Model model) {
        Settings settings = Settings.builder()
                ...
                .put("index.refresh_interval", "10s").build();
        return settings;
    }
    ...
    protected Mappings createMapping(Model model) throws IOException {
        ...
        dynamicTemplates.add(
                Collections.singletonMap("strings_as_keyword",
                        new DynamicTemplate("string", null, null, null, null, null, Collections.singletonMap("type", "keyword"))));
        return mappings;
    }
    ...
    private Map<String, Object> createProperties(Model model) {
        ...
        for (ModelColumn columnDefine : model.getColumns()) {
            final String type = dataTypeMapping.transform(columnDefine.getType(), columnDefine.getGenericType());
