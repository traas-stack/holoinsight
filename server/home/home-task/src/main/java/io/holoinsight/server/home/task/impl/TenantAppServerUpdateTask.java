...
public class TenantAppServerUpdateTask extends AbstractMonitorTask {
    ...
    private void syncAoAction() {
        ...
        for (MetaTableDTO metaTableDTO : all) {
            ...
            Debugger.print("TenantAppServerUpdateTask", "qurey meta list from table={} size={}", metaTableDTO.name,
                    mapList.size());
            ...
            if ((serverUkSets.size() * 1.0 / mapList.size()) > 0.8) {
                log.warn("need delete expried list from table={} size={} mapList={}, is fusing", metaTableDTO.name,
                        serverUkSets.size(), mapList.size());
                continue;
            }
