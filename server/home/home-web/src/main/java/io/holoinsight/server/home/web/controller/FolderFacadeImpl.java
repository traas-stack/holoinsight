...
public class FolderFacadeImpl extends BaseFacade {
    ...
    public JsonResult<Object> update(@RequestBody Folder folder) {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void checkParameter() {
                ...
                ParaCheckUtil.checkEquals(folder.getTenant(),
                        RequestContext.getContext().ms.getTenant(), "tenant is illegal");

                Folder item = folderService.queryById(folder.getId(), RequestContext.getContext().ms.getTenant());
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                userOpLogService.append("folder", String.valueOf(folder.getId()), OpType.CREATE,
                        mu.getLoginName(), ms.getTenant(), J.toJson(folder), null, null,
                        "folder_create");
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                if (!CollectionUtils.isEmpty(byMap)) {
                    JsonResult.createFailResult(
                            String.format("there are %s conf under this folder", byMap.size()));
                    return;
                }

                Folder byId = folderService.queryById(id,
    ...
    public JsonResult<Boolean> updateParentFolderId(@RequestBody Folder folder) {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                Folder update = folderService.queryById(folder.id, RequestContext.getContext().ms.getTenant());
                if (null == update) {
                    throw new MonitorException(ResultCodeEnum.CANNOT_FIND_RECORD, "can not find record");
                }
                if (null != mu) {
    ...
    @MonitorScopeAuth(targetType = AuthTargetType.TENANT, needPower = PowerConstants.VIEW)
    public JsonResult<List<Folder>> queryByParentFolderIdAndTenant(@PathVariable("parentFolderId") Long parentFolderId) {
