    ...
    public static XHttpResponse sendWebhook(WebhookInfo webhookInfo) {
        try {
            ...
            switch (webhookInfo.getRequestType().toUpperCase()) {
                case "POST":
                    xHttpRequest = XHttpRequest.post(webhookInfo.getRequestUrl(), G.get().fromJson(webhookInfo.getRequestHeaders(), Map.class),
                            "UTF-8", 60000, webhookInfo.getWebhookMsg().getBytes(), "UTF_8");
                    break;
                case "GET":
                    xHttpRequest = XHttpRequest.get(webhookInfo.getRequestUrl(), null, G.get().fromJson(webhookInfo.getRequestHeaders(), Map.class), "UTF-8", 60000);
                    break;
                case "PUT":
                    xHttpRequest = XHttpRequest.put(webhookInfo.getRequestUrl(), G.get().fromJson(webhookInfo.getRequestHeaders(), Map.class), "UTF_8",null,"UTF-8", 60000);
                    break;
                case "DELETE":
                    xHttpRequest = XHttpRequest.delete(webhookInfo.getRequestUrl(), G.get().fromJson(webhookInfo.getRequestHeaders(), Map.class), "UTF-8", 60000);
                    break;
                default:
                    xHttpRequest = XHttpRequest.get(webhookInfo.getRequestUrl(), null, G.get().fromJson(webhookInfo.getRequestHeaders(), Map.class), "UTF-8", 60000);
            }
