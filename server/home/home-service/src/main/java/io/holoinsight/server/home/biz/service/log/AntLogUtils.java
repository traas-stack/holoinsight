...
public class AntLogUtils {
  ...
  public static String buildQuery(String query, ArrayList<String> instances, String logPath) {
    ...
    if (!CollectionUtils.isEmpty(instances)) {
      for (int index = 0; index < instances.size(); index++) {
        if (index == 0) {
          dest = new StringBuffer(dest).append(" and ").append(LogConstants.SLS_KEY_HOST_NAME)
              .append(":").append(instances.get(index)).toString();
        } else {
          dest = new StringBuffer(dest).append(" or ").append(LogConstants.SLS_KEY_HOST_NAME)
              .append(":").append(instances.get(index)).toString();
        }
      }
    }
    ...
    // 增加日志路径选择
    dest = new StringBuffer(dest).append(" and ").append(LogConstants.SLS_KEY_PATH).append(":")
        .append(logPath).toString();

    return dest;
  }

  public static String buildAntLogInfoCacheKey(String region, String appId, String envId,
      String serviceId, String logPath) {
  ...
  public static String buildAntLogInfoApp(String appId, String envId, String serviceId) {
    if (StringUtils.isEmpty(appId) || StringUtils.isEmpty(envId)
        || StringUtils.isEmpty(serviceId)) {
      throw new InvalidParameterException("buildAntLogInfoApp params can not be null");
    }
    return new StringBuffer().append(appId).append("_").append(envId).append("_").append(serviceId)
        .toString();
  }
  ...
  public static String buildAntLogInfoCacheKey(String region, String app, String logPath) {
    ...
    return new StringBuffer().append(region).append("@").append(app).append("@").append(logPath)
        .toString();
  }
