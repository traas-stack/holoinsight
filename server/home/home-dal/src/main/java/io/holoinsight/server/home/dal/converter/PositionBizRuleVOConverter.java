...
public class PositionBizRuleVOConverter {
    ...
    public static PositionBizRuleVO dtoToVO(PositionBizRuleDTO positionBizRuleDTO) {
        ...
        if (!checkErrorCodeConfigMvel(errorCodeConfig)) {
            ...
            positionBizRuleVO.setBizResult(getErrorCodeConfigMvelForVO((errorCodeConfig.split(TRIPLE_AT)[0])));
    ...
    public static List<BizResultConfig> getErrorCodeConfigMvelForVO(String bizResMvelStr) {
        ...
        for (String bizResMvel : bizResMvelStr.split(TRIPLE_AND)) {
            ...
            bizResultConfig.setFieldType(FieldTypeEnum.getFieldTypeByCode
                    (bizResMvel.split(TRIPLE_SHARP)[1]).getValue());

            bizResultConfig.setOperateType(OperateTypeEnum.getOperateTypeByCode
                    (bizResMvel.split(TRIPLE_SHARP)[2]).getValue());
    ...
    public static String getErrorCodeConfigStr(PositionBizRuleVO positionBizRuleVO) {
        ...
        List<String> bizResMvelList = positionBizRuleVO.getBizResult().stream().map(item -> {
            ...
            sb.append(FieldTypeEnum.getFieldTypeByValue
                    (item.getFieldType()).getCode()).append(TRIPLE_SHARP);
            sb.append(OperateTypeEnum.getOperateTypeByValue
                    (item.getOperateType()).getCode()).append(TRIPLE_SHARP);
            sb.append(item.getFieldValue());
            return sb.toString();
        }).collect(Collectors.toList());
        return StringUtils.join(bizResMvelList, TRIPLE_AND) + TRIPLE_AT + positionBizRuleVO.getErrorCode();
    }
}
