...
public class GrpcFacadeTemplate {
    ...
    public void manage(StreamObserver<DataBaseResponse> responseObserver,
                       GrpcManageCallback callback, DataBaseResponse.Builder builder) {
        ...
        try {
            ...
            String trace = builder.getTraceId() + ", serverResult=[" + build.getSuccess() + "], serverCost=["
                           + stopWatch.getTime() + "]";
            log.info(trace);
            builder.setTraceId(trace + ", targetIp=[" + AddressUtil.getLocalHostIPV4() + "], "
                               + build.getErrMsg());
