...
public class TemplateValue {
   ...
   * @return
   */
  public static List<TemplateValue> convertFirstAlertNotify(AlertNotify alertNotify,
      String domain) {
    ...
    if (alertNotify.isPqlNotify()) {
      ...
      templateValue.setAlarmContent(AlarmContentGenerator.genPqlAlarmContent(
      ...
      templateValue.setAlarmContent(
          notifyDataInfo.getTriggerContent() + "当前值为：" + notifyDataInfo.getCurrentValue());
    }
    templateValue.setAggregationNum(String.valueOf(alertNotify.getAggregationNum()));
    templateValue
        .setRuleUrl(domain + templateValue.getRuleId() + "?tenant=" + templateValue.getTenant());
