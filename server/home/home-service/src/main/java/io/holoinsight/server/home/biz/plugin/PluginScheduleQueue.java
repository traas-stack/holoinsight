...
public class PluginScheduleQueue {
    ...
    ScheduledExecutorService syncExecutorService = new ScheduledThreadPoolExecutor(7,
            r -> new Thread(r, "ScheduleQueue"));

    ThreadPoolExecutor workExecutor = new ThreadPoolExecutor(10, 10, 1, TimeUnit.HOURS, new ArrayBlockingQueue<>(100), new ThreadFactory() {
        @Override
    ...
    @PostConstruct
    public void start(){
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_5_sec, ScheduleTimeEnum.WAIT_5_SEC), 5, 5, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_15_sec, ScheduleTimeEnum.WAIT_15_SEC), 5, 15, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_30_sec, ScheduleTimeEnum.WAIT_30_SEC), 5, 30, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_45_sec, ScheduleTimeEnum.WAIT_45_SEC), 5, 45, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_1_min, ScheduleTimeEnum.WAIT_1_MIN), 5, 60, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_5_min, ScheduleTimeEnum.WAIT_5_MIN), 5, 300, TimeUnit.SECONDS);
        syncExecutorService.scheduleAtFixedRate(new Worker(queue_10_min, ScheduleTimeEnum.WAIT_10_MIN), 5, 600, TimeUnit.SECONDS);
    }
