...
public abstract class ScheduleLoadTask implements Runnable {
    ...
    private static final ThreadFactory            factory           = new BasicThreadFactory.Builder()
        .namingPattern("schedule-load-%d").daemon(true).build();
    private static final ScheduledExecutorService scheduledExecutor = Executors
        .newScheduledThreadPool(5, factory);
     ...
     * @param task
     */
    public static synchronized void registerTask(ScheduleLoadTask task,
                                                 Boolean now) throws Exception {
