...
public interface MetricApi {
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "请求正常。", response = List.class),
    ...
    @ApiOperation(
            value = "query metric data",
            nickname = "queryMetricData",
            notes = "查询指标数据",
            response = MetricValues.class,
            authorizations = {
                    @Authorization(value = "APIKeyHeader"),
                    @Authorization(value = "APIKeyQueryParam")
            },
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "请求正常。", response = MetricValues.class),
                    @ApiResponse(code = 400, message = "请求失败。", response = FailResponse.class)
            })
    @RequestMapping(
            value = "/query",
            ...
            consumes = {"application/json"},
            method = RequestMethod.POST)
    ResponseEntity<MetricValues> queryMetricData(
            @ApiParam(value = "metric查询条件。", required = false) @Valid @RequestBody
            QueryMetricRequest request) throws IOException;

    @ApiOperation(
            value = "query metric schema",
            nickname = "queryMetricSchema",
            notes = "查询指标结构",
            response = MetricValues.class,
            authorizations = {
                    @Authorization(value = "APIKeyHeader"),
    ...
    @RequestMapping(
            ...
            consumes = {"application/json"},
            method = RequestMethod.POST)
    ResponseEntity<List<String>> queryMetricSchema(
            @ApiParam(value = "metric name", required = false) @RequestBody @Valid QueryMetricRequest metric) throws IOException;
}