...
public class AgentService {
    ...
    private static final StatAccumulator<StringsKey> AGENT_HEARTBEAT_STAT = MetricsUtils.SM.create("agent.heartbeat");
    ...
    private void notifyProd(AuthInfo a, String agentId) {
        ...
        if (agent != null && (agent.getJson().getMode().equals(Agent.MODE_SIDECAR))) {
            ...
            try {
                ...
                long end = System.currentTimeMillis();
                LOGGER.info("[prod] [agentInsertOrUpdate] update [{}] row={} cost=[{}]", table, row, end - begin);
            }
        }
    }
    ...
    private void registerAgent0(AuthInfo ai, RegisterAgentRequest request) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (agent == null) {
                GaeaAgentDO gado = mapper.selectOneByExampleWithBLOBs(GaeaAgentDOExample.newAndCreateCriteria() //
            ...
            if (insert) {
                ...
                GaeaAgentDO ga = mapper.selectOneByExampleWithBLOBs(GaeaAgentDOExample.newAndCreateCriteria() //
                    .andAgentIdEqualTo(agentId) //
                    .example());//
    ...
    public synchronized void markDeleteExpiredAgents() {
        // 删除过期 agents 数据
        Date expiredDate = new Date(System.currentTimeMillis() - agentConfig.getMaintain().getExpire().toMillis());
        GaeaAgentDOExample example = GaeaAgentDOExample.newAndCreateCriteria() //
    ...
    private void markDeleteExpiredAgentsForDaemonAgent() {
        ...
        for (Agent a : agentStorage.readonlyList()) {
            ...
            if (Agent.MODE_DAEMONSET.equals(mode) || Agent.MODE_CENTRAL.equals(mode) || Agent.MODE_CLUSTERAGENT.equals(mode)) {
        ...
        if (expiredAgentIds.size() > 0) {
            ...
            record.setGmtModified(new Date());
            record.setStatus(1);
            GaeaAgentDOExample example = GaeaAgentDOExample.newAndCreateCriteria().andAgentIdIn(expiredAgentIds).example();
            int count = mapper.updateByExampleSelective(record, example, //
                GaeaAgentDO.Column.status, GaeaAgentDO.Column.gmtModified);
            LOGGER.info("mark delete expired daemonset agents, count=[{}], agentIds={}", count, expiredAgentIds);
    ...
    public void deleteDeletedAgents() {
        // 删除过期agents数据
        Date date = new Date(System.currentTimeMillis() - agentConfig.getMaintain().getExpire().toMillis());
        GaeaAgentDOExample example = GaeaAgentDOExample.newAndCreateCriteria() //
