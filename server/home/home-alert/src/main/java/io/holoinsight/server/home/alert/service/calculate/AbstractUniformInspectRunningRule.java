...
public class AbstractUniformInspectRunningRule {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AbstractUniformInspectRunningRule.class);
    ...
    public EventInfo eval(ComputeContext context) {
        ...
        try {
            ...
            logger.error("fail to eval inspectConfig {}, traceId: {} ", G.get().toJson(inspectConfig), traceId, ex);
        }
        return events;
    }

    public EventInfo runPqlRule(InspectConfig inspectConfig, long period) {
        if (inspectConfig.getIsPql() && !CollectionUtils.isEmpty(inspectConfig.getPqlRule().getDataResult())) {
    ...
    public EventInfo runRule(InspectConfig inspectConfig, long period) {
        ...
        if ((BoolOperationEnum.AND.equals(rule.getBoolOperation()) && rule.getTriggers().size() == triggerMap.size())
     ...
     */
    public static TriggerResult apply(DataResult dataResult, ComputeInfo computeInfo, Trigger trigger) {
    ...
    private static FunctionConfigParam buildFunctionConfigParam(ComputeInfo computeInfo, Trigger trigger) {
