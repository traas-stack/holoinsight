...
public class TenantIntegrationGeneratedTask extends AbstractMonitorTask {
    ...
    private Map<String, Long> getFromDb() {
        ...
        for (TenantDTO tenantDTO : tenantDTOS) {
            List<IntegrationGenerated> dblist = integrationGeneratedService
                .queryByTenant(tenantDTO.getCode());
            if (CollectionUtils.isEmpty(dblist)) {
                continue;
            }

            for (IntegrationGenerated generated : dblist) {
                dbUks.put(String.format("%s_%s_%s_%s", generated.getTenant(),
        ...
        for (TenantDTO tenantDTO : tenantDTOS) {
            ...
            for (Map.Entry<String, String> entry : dbAppMaps.entrySet()) {
                ...
                if (entry.getValue().equalsIgnoreCase("VM")) {
                    generateds
                        .add(generated(tenantDTO.getCode(), entry.getKey(), "vmsystem", "System"));
                } else {
                    generateds
                        .add(generated(tenantDTO.getCode(), entry.getKey(), "podsystem", "System"));
                }

                generateds.add(
                    generated(tenantDTO.getCode(), entry.getKey(), "logpattern", "LogPattern"));

                generateds
                    .add(generated(tenantDTO.getCode(), entry.getKey(), "portcheck", "PortCheck"));
            }
            ...
            for (IntegrationPluginDTO integrationPluginDTO : integrationPluginDTOS) {
                ...
                if (null == plugin)
                    continue;
                List<AbstractIntegrationPlugin> abstractIntegrationPlugins = plugin
                    .genPluginList(integrationPluginDTO);
                ...
                for (String app : appSets) {
                    generateds.add(generated(tenantDTO.getCode(), app,
                        integrationPluginDTO.getProduct().toLowerCase(),
    ...
    private Set<String> getCollectApps(CloudMonitorRange cloudMonitorRange) {
        ...
        if (CollectionUtils.isEmpty(cloudMonitorRange.condition)) {
            List<Map<String, Object>> metaList = dataClientService
                .queryAll(cloudMonitorRange.getTable());
            ...
            List<Map<String, Object>> metaList = dataClientService
                .queryByExample(cloudMonitorRange.getTable(), queryExample);
            if (CollectionUtils.isEmpty(metaList))
                return new HashSet<>();
            mapList.addAll(metaList);
        }
    ...
    private Map<String, String> getDbApps(String tableName) {
        ...
        CommonLocalCache.put(MD5Hash.getMD5(INTEGRATION_GENERATED_CACHE_KEY + tableName), appMaps,
            10, TimeUnit.MINUTES);
