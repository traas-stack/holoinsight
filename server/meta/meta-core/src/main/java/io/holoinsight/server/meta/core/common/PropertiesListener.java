...
public class PropertiesListener implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    public void onApplicationEvent(ApplicationEnvironmentPreparedEvent applicationEnvironmentPreparedEvent) {
        // 读取配置环境
        String env = null;
        if (!ArrayUtils.isEmpty(applicationEnvironmentPreparedEvent.getEnvironment().getActiveProfiles()) &&
                StringUtils.hasText(applicationEnvironmentPreparedEvent.getEnvironment().getActiveProfiles()[0])) {
        ...
        if (CollectionUtils.isEmpty(allProperty)) { return; }
