...
public class HttpPqlQueryService implements PqlQueryService {
  ...
  private Map<String, Object> parasParam(PqlParam pqlParam) {
    ...
    try {
      ...
      LOGGER.error("[PqlRemoteQuery] encode pql query param error , pql : {}", pqlParam.getQuery(), e);
  ...
  private List<Result> toResult(PqlParam pqlParam, String body) {
    ...
    if (Objects.isNull(data)) {
      LOGGER.error("[PqlRemoteQuery] exec pql: {} get empty data, resp body:{}", pqlParam.getQuery(), body);
      return results;
    }
    List<Map<String, Object>> items = data.getResult();
    if (CollectionUtils.isEmpty(items)) {
      LOGGER.error("[PqlRemoteQuery] exec pql: {} get empty result, resp body:{}", pqlParam.getQuery(), body);
      return results;
    }
    items.forEach(item -> {
