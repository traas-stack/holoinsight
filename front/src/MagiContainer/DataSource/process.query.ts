...
export function getTime(time: number, interval = MINUTE) {
  return Math.floor(time / interval) * interval - (interval % DAY === 0 ? 8 * HOUR : 0);
}
...
export function merge(targets: DataSourceTarget[], ctx: Context): DataSourceTargetWithContetxt[] {
...
export function getDataTable(target?: CustomTarget): DataTable | undefined {
  ...
  return Array.isArray(tables) ? tables.find((item) => item.table === groupbyName) : tables;
}
...
export function processFilters(
  ...
  if (filters && filters.length > 0 && labels && labels.length) {
    const availables = filters.filter(
      (item) => item && item.dimKey && item.filterType && item.values && item.values.length > 0,
    );
...
export function CF2PF(filters: Filter[]): PFilter[] | undefined {
  const availables: Filter[] = (filters || []).filter(
    (item: any) => item && item.dimKey && item.filterType && item.values && item.values.length > 0,
  );
  if (availables.length > 0) {
...
export function PF2CF(filters: PFilter[]): Filter[] | undefined {
  ...
  if (availables.length > 0) {
    return availables.map((item) => {
      ...
      return { dimKey: name, filterType: 'NotRegexpMatch', values: [`^(${value})$`] };
    });
  }
  return undefined;
}
...
export function getFields(metrics: string[]): string[] {
  return uniq(metrics.map((item) => item.replace(/_(non|wow)_(value|percent)$/, '')));
}
...
export function process(info: CustomTarget & Context): Condition[] {
  ...
  if (pluginType === 'patternMatch') {
    if (table.interval === SECOND) {
      params.plugin = 'PMSEC';
      if (params.groupbyName) params.groupbyName = params.groupbyName.split('@@')[0];
    }
  }
...
export function foldPROMQL(targets: DataSourceTarget[]): any[] {
  ...
  return targets.map((item) => {
    if (item.type === 'PROMQL' && !item.folded) {
      ...
      const result = {
        ...
        refs: (refs || [])
          ...
          .map((ref: any) => {
            ...
            if (type === 'CUSTOM') {
              const { bizDomainId, outsideDomainId, workspaceId, tenantId, dsId, pluginType } =
                source;
              const params = {
                name,
                groupBy: groupbyName,
      ...
      result.step = result.refs.every((item: any) => isSecondSource(item.pluginType))
        ? 1000
        : 60000;
      return result;
    }
