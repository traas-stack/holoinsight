...
public class CentralAgentService {
    ...
    public void refresh() {
        ...
        for (Agent a : agentStorage.readonlyLoop()) {
            if (a.getLastHeartbeat().after(expireTime) && Agent.MODE_CENTRAL.equals(a.getJson().getMode())) {
    ...
    public static class State {
        ...
        public void build() {
            ...
            for (ClusterState c : clusters.values()) {
                c.agents.values().removeIf(a -> a.getLastHeartbeat().before(expireTime) || a.getJson().getHostname() == null);
                c.build();
            }
        }
    }
