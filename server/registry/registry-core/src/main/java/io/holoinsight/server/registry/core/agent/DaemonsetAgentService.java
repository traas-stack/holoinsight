    ...
    public class State {
        ...
        State(int ignored) {
            for (Agent a : agentStorage.readonlyLoop()) {
                ...
                DaemonsetAgent.Key key = new DaemonsetAgent.Key(a.getTenant(), a.getJson().getK8s().getHostIP());
                ...
                LOGGER.warn("[daemonset] key={} has multi agents {} {}", key, JsonUtils.toJson(a), JsonUtils.toJson(oldA));
            }
        }
    }
}
