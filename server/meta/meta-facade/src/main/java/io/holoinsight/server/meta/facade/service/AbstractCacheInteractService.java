...
public abstract class AbstractCacheInteractService {
    ...
    protected <T> T tryUntilSuccess(Function<DimCookie, T> function, String method, int i) {
        try {
            ...
            logger.error(e.getMessage(), e);
            if (i >= MAX_RETRY) {
                String message = String.format("request server fail and reach the max retry times %s, server=%s, method=%s, msg=%s.",
    ...
    protected synchronized void resetCookie() {
        logger.info("reset cookie start, channel authority={}", this.cookie == null ? null : this.cookie.getChannel().authority());
        if (this.cookie != null && this.cookie.isExpired()) {
            this.cookie.destory();
        }
        DimCookie newCookie = initCookie();
        this.cookie = newCookie;
        logger.info("reset cookie finish, channel authority={}", this.cookie == null ? null : this.cookie.getChannel().authority());
    }
    ...
    protected DimCookie initCookie() {
        ...
        if (CollectionUtils.isEmpty(servers)) { return cookie; }
