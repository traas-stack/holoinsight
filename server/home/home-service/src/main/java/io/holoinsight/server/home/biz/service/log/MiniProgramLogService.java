...
public class MiniProgramLogService {
   ...
   */
  public MiniProgramLogCountQueryResponse queryMiniProgramLogCount(String appId, String envId,
      MiniProgramLogCountQueryRequest request)
      throws LogException, URISyntaxException, HttpException {
    String app = AntLogUtils.buildAntLogInfoApp(appId, envId, request.getServiceId());

    AntLogInfo info = null;
    String logPath = "";

    if (StringUtils.isEmpty(request.getLogPath())
        || request.getLogPath().equals(LogConstants.MINI_PROGRAM_LOG_PATH_STDOUT)) {
    ...
    GetLogsResponse slsLogResponse = slsLogService.getGetLogsResponse(info.getProjectName(),
        info.getLogstoreName(), request.getFrom(), request.getTo(), query, 0, 100, false);
   ...
   * @throws HttpException
   */
  public MiniProgramLogQueryResponse queryMiniProgramLog(String appId, String envId,
      MiniProgramLogQueryRequest request) throws LogException, URISyntaxException, HttpException {
    String app = AntLogUtils.buildAntLogInfoApp(appId, envId, request.getServiceId());
    AntLogInfo info = null;
    String logPath = "";
    ...
    GetLogsResponse slsLogResponse = slsLogService.getGetLogsResponse(info.getProjectName(),
        info.getLogstoreName(), request.getFrom(), request.getTo(), query, offset,
        request.getPageSize(), request.isReverse());
    return convert(slsLogResponse, appId, envId, request);
  }
   ...
   * @throws HttpException
   */
  public MiniProgramLogContextQueryResponse queryMiniProgramLogContext(String appId, String envId,
      MiniProgramLogContextQueryRequest request)
      throws LogException, URISyntaxException, HttpException {
    String app = AntLogUtils.buildAntLogInfoApp(appId, envId, request.getServiceId());
    ...
    GetContextLogsResponse slsLogContextResponse = slsLogService.getSlsContextLogs(
        info.getProjectName(), info.getLogstoreName(), request.getPackID(), request.getPackMeta(),
        request.getBackLines(), request.getForwardLines());
   ...
   * @throws HttpException
   */
  public List<String> queryMiniProgramLogPath(String appId, String envId, String serviceId,
      Integer from, Integer to) throws URISyntaxException, HttpException, LogException {
    String app = AntLogUtils.buildAntLogInfoApp(appId, envId, serviceId);
    AntLogInfo logInfo =
        antLogService.queryCustomLogInfo(LogConstants.MINI_PROGRAM_LOG_REGION, app);

    // 这里小程序云默认开启了__tag__:__path__作为日志索引
    String query = "* | select distinct \"__tag__:__path__\"";
    Integer offset = 0;
    GetLogsResponse slsLogResponse = slsLogService.getGetLogsResponse(logInfo.getProjectName(),
        logInfo.getLogstoreName(), from, to, query, offset, 100, false);
  ...
  public MiniProgramLogCountQueryResponse convert(GetLogsResponse source, String appId,
      String envId, MiniProgramLogCountQueryRequest request) {
  ...
  public static MiniProgramLogContextQueryResponse convert(GetContextLogsResponse source,
      String appId, String envId, MiniProgramLogContextQueryRequest request) {
  ...
  public static MiniProgramLogQueryResponse convert(GetLogsResponse source, String appId,
      String envId, MiniProgramLogQueryRequest request) {
