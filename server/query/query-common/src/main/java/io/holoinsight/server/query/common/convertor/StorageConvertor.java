...
public class StorageConvertor {
  ...
  public static QueryProto.Span convertSpan(Span span) {
    ...
    if (CollectionUtils.isNotEmpty(tags)) {
      spanBuilder.addAllTags(Iterables.transform(tags, StorageConvertor::convertKeyValue));
    }

    List<LogEntity> logs = span.getLogs();
    if (CollectionUtils.isNotEmpty(logs)) {
      spanBuilder.addAllLogs(Iterables.transform(logs, StorageConvertor::convertLogEntity));
    }
  ...
  public static QueryProto.LogEntity convertLogEntity(LogEntity logEntity) {
    ...
    if (CollectionUtils.isNotEmpty(datas)) {
      logBuilder.addAllData(Iterables.transform(datas, StorageConvertor::convertKeyValue));
    }
