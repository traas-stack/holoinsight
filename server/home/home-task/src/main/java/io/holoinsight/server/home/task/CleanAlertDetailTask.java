    ...
    try {
      ...
      while (keepDelete) {
        ...
        if (pageRes == null || CollectionUtils.isEmpty(pageRes.getRecords())) {
          ...
          List<Long> ids = pageRes.getRecords().stream().map(AlarmHistoryDetail::getId)
      ...
      LOGGER.error("[CleanAlertDetailTask] fail_to_delete_alarm_detail before {} sum {} for {}",
          date.toString(), deleteSum, e.getMessage(), e);
    } finally {
      LOGGER.info("[CleanAlertDetailTask] delete_alarm_detail before {} sum {}", date.toString(),
          deleteSum);
    }
  }
