...
public class CollectTemplateSyncer {
    ...
    public void init() {
        ...
        scheduler2 = new ScheduledThreadPoolExecutor(1, new OneThreadFactory("template-maintainer-trigger"));
    }
    ...
    public Mono<Void> initApplyTemplates() {
        ...
        return Flux.fromIterable(templates) //
            .flatMap(t -> { //
                return taskMaintainer.maintainTemplateDelta(CollectTemplateDelta.one(t)).doOnTerminate(() -> scheduleOnce(t));
            }, concurrency) //
            .ignoreElements() //
            .doOnSuccess(ignored -> { //
    ...
    public void startSync() {
        ...
        // 开始的时候多延迟一会
        deltaSyncTaskDisposable = DynamicScheduledExecutorService.wrap(scheduler).dynamic(this::deltaSync, //
    ...
    private synchronized void fullSync() {
        ...
        List<Long> ids = readCommittedTT.execute(status -> {
            ...
            // 我只需要id即可
            List<GaeaCollectConfigDO> configDos = mapper.selectByExampleSelective(example, GaeaCollectConfigDO.Column.id);
        ...
        // TODO 名字能不能好好起
        if (selectIds.size() > 0) {
            List<GaeaCollectConfigDO> tempDos = mapper.selectByExampleSelective(GaeaCollectConfigDOExample.newAndCreateCriteria() //
                    .andDeletedEqualTo(0) //
                    .andIdIn(selectIds) //
                    .example(), //
        ...
        for (CollectTemplate t : memory.values()) {
            CollectTemplateDelta d = templateDeltas.computeIfAbsent(t.getTableName(), CollectTemplateDelta::new);
            ...
            if (gcc == null || gcc.getDeleted() > 0) {
                ...
                // 不可能走到这里
                // select id from ... where deleted = 0 里没有它, 但 select * from ... where id in ... 缺有它 deleted == 0
        ...
        int buildError = 0;
        if (needRetry.size() > 0) {
            List<GaeaCollectConfigDO> tempDos = mapper.selectByExampleWithBLOBs(GaeaCollectConfigDOExample.newAndCreateCriteria() //
        ...
        LOGGER.info("full sync success ids=[{}] add=[{}] del=[{}] error=[{}]", ids.size(), add, del, buildError);
        ...
        // 当然这个算法也有缺点, 就是它需要一口气select出所有的ids, 这个sql简直太简单 select id from gaea_collect_config where deleted = 0;
    ...
    private Map<String, CollectConfigDelta> retry(Collection<String> tableNames) {
        GaeaCollectConfigDOExample retryExample = GaeaCollectConfigDOExample.newAndCreateCriteria() //
            .andTableNameIn((tableNames instanceof List) ? (List) tableNames : new ArrayList<>(tableNames)) //
            .example(); //
    ...
    private void applyDeltas(Map<String, CollectTemplateDelta> templateDeltas) {
        ...
        for (val d : templateDeltas.values()) {
            for (val t : d.getAdd()) {
                LOGGER.info("delta add [{}/{}] dbTime={}", t.getId(), t.getTableName(), sdf.format(t.getGmtModified()));
            }
            for (val t : d.getDelete()) {
                LOGGER.info("delta del [{}/{}] dbTime={}", t.getId(), t.getTableName(), sdf.format(t.getGmtModified()));
            }
        }
    ...
    private Map<String, CollectTemplateDelta> syncDataOnce0(Date begin, Date end) throws Throwable {
        ...
        if (configDos.isEmpty()) {
            LOGGER.info("delta sync success range=[{}, {}) empty, dbCost=[{}]", SDF.format(begin), SDF.format(end), dbEnd - dbBegin);
            return null;
        }
        ...
        for (Map.Entry<String, CollectConfigDelta> e : groups.entrySet()) {
            ...
            // 1.2 否则add==1, 如果此时内存里确实没有该tableName的template, 那么这确实是一次add {缺点是这里需要有一个byTableName的索引}, 建议都走重新select
        ...
        if (retryTableNames.size() > 0) {
            ...
            groups2.entrySet().removeIf(e -> e.getValue().getStatus() == CollectConfigDelta.STATUS_INVALID);
        ...
        LOGGER.info("delta sync success range=[{}, {}) update=[{}] add=[{}] delete=[{}] retry=[{}] invalid=[{}]", //
    ...
    private Map<String, CollectTemplateDelta> buildCollectTemplateDelta(Map<String, CollectConfigDelta> groups) {
        // 同步完之后开始处理
        // return new CollectTemplateDelta(groups);
        Map<String, CollectTemplateDelta> templateDeltas = Maps.newHashMapWithExpectedSize(groups.size());
        for (Map.Entry<String, CollectConfigDelta> e : groups.entrySet()) {
    ...
    private Map<String, CollectConfigDelta> toGroups(List<GaeaCollectConfigDO> configDos, boolean warnNotExistDelete) {
    ...
    private void triggerMaintainTemplate(CollectTemplate t) {
        taskMaintainer.publish(t, () -> {
            ...
            // 然后我们才投递一个maintainTemplate, 此时可能会有问题, 因为按照我们的线程模型前者一定会先执行, 我们再执行, 可能会让原本该倍deleted的targets又被创建
