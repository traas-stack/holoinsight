...
public class MigrateService {
    ...
    public CustomPluginDTO migrateCustomPlugin(Long id, String sourceTenant, String targetTenant) {
        ...
        while (currentDeep < MAX_COPY_FOLDER_DEEP) {
            ...
            if (folder == null) {
                throw new MonitorException("invalid folder status [folderId: " + parentFolderId + "]" + "[tenant: " + sourceTenant + "]");
            }
        ...
        for (AlarmSubscribeInfo info : alarmSubscribeDTO.getAlarmSubscribe()) {
            ...
            for (String type : types) {
                ...
                if (type.equals("dingDingRobot")) {
                    ...
                    AlarmDingDingRobotDTO alarmDingDingRobotDTO = migrateDingDingRobot(dingDingRobotId, sourceTenant, targetTenant);
                    info.setSubscriber(String.valueOf(alarmDingDingRobotDTO.getId()));
                    alarmSubscribeService.updateById(info);
                }
                if (type.equals("webhook")) {
                    Long webhookId = info.getGroupId();
                    AlarmWebhookDTO alarmWebhookDTO = migrateAlarmWebhook(webhookId, sourceTenant, targetTenant);
     ...
     * @return
     */
    public AlarmDingDingRobotDTO migrateDingDingRobot(Long id, String sourceTenant, String targetTenant) {
        AlarmDingDingRobotDTO alarmDingDingRobotDTO = alarmDingDingRobotService.queryById(id, sourceTenant);
