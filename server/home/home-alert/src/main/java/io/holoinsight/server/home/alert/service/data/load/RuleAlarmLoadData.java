...
public class RuleAlarmLoadData implements AlarmLoadData {
    ...
    private QueryProto.QueryResponse queryData(ComputeTask computeTask, InspectConfig e, Trigger trigger) {
    ...
    protected QueryProto.QueryRequest buildRequest(long timestamp, String tenant, Trigger trigger) {
        ...
        for (DataSource dataSource : trigger.getDatasources()) {
            ...
            if (StringUtils.isNotEmpty(aggregator)
                    && StringUtils.isBlank(downsample)
                    && !aggregator.equals("none")) {
                downsample = "1m";
            }
            long start = timestamp
                    - (trigger.getStepNum() - 1) * time * PeriodType.MINUTE.intervalMillis()
                    - trigger.getDownsample() * PeriodType.MINUTE.intervalMillis();
            long end = timestamp + time * PeriodType.MINUTE.intervalMillis();
            QueryProto.SlidingWindow slidingWindow = QueryProto.SlidingWindow.newBuilder()
                    .setAggregator(trigger.getAggregator())
                    .setWindowMs(trigger.getDownsample() * 60 * 1000)
                    .build();
