...
public class IntegrationProductFacadeImpl extends BaseFacade {
    ...
    public JsonResult<Object> update(@RequestBody IntegrationProductDTO integrationProductDTO) {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                IntegrationProductDTO update = integrationProductService.updateByRequest(integrationProductDTO);
    ...
    @ResponseBody
    @MonitorScopeAuth(targetType = AuthTargetType.TENANT, needPower = PowerConstants.EDIT)
    public JsonResult<IntegrationProductDTO> save(@RequestBody IntegrationProductDTO integrationProductDTO) {
        final JsonResult<IntegrationProductDTO> result = new JsonResult<>();
        facadeTemplate.manage(result, new ManageCallback() {
            @Override
            public void checkParameter() {
            ...
            public void doManage() {
                ...
                assert mu != null;
                userOpLogService.append("integration_product", String.valueOf(save.getId()),
                        OpType.CREATE, mu.getLoginName(), ms.getTenant(), J.toJson(integrationProductDTO),
    ...
    public JsonResult<List<IntegrationProductDTO>> queryByName(@PathVariable("name") String name) {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            @Override
            public void doManage() {
                List<IntegrationProductDTO> integrationProductDTOs = integrationProductService.findByMap(
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                if (!CollectionUtils.isEmpty(integrationProductDTOs)) {
                    ...
                    QueryProto.QueryRequest.Builder builder = QueryProto.QueryRequest.newBuilder().setTenant(tenant);
                    for (String type : metrics.getSubMetrics().keySet()) {
                        for (IntegrationMetricDTO metric : metrics.getSubMetrics().get(type)) {
                            builder.addDatasources(QueryProto.Datasource.newBuilder().setMetric(metric.getName()).setStart(from).setEnd(now)
                                    .build());
                        }
                    }
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                ...
                integrationPluginDTOs.parallelStream().forEach(plugin -> {
                    ...
                    List<IntegrationProductDTO> integrationProductDTOs = integrationProductService.findByMap(
                            Collections.singletonMap("name", productName));
                    if (!CollectionUtils.isEmpty(integrationProductDTOs)) {
                        IntegrationProductDTO integrationProductDTO = integrationProductDTOs.get(0);
                        IntegrationMetricsDTO metrics = integrationProductDTO.getMetrics();
                        List<String> metricNames = metrics.getSubMetrics().values().stream().flatMap(v -> v.stream()).map(
                                IntegrationMetricDTO::getName).collect(Collectors.toList());
                        ;
                        ...
                        for (String metricName : sampleNames) {
                            builder.addDatasources(QueryProto.Datasource.newBuilder().setMetric(metricName).setStart(from).setEnd(now)
                                    .build());
                        }
                        QueryResponse response = queryClientService.queryTags(builder.build());
                        List<Result> results = response.getResults();
    ...
    public JsonResult<List<IntegrationProductDTO>> listAll() {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            @Override
            public void doManage() {
                List<IntegrationProductDTO> integrationProductDTOs = integrationProductService.findByMap(Collections.emptyMap());
    ...
    public JsonResult<List<String>> listAllNames() {
        ...
        facadeTemplate.manage(result, new ManageCallback() {
            ...
            public void doManage() {
                List<IntegrationProductDTO> integrationProductDTOs = integrationProductService.findByMap(Collections.emptyMap());
                List<String> names = integrationProductDTOs == null ? null : integrationProductDTOs.stream().map(
                        IntegrationProductDTO::getName).collect(
                        Collectors.toList());
