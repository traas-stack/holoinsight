...
public class MultiFutureTask {
    ...
    static final class Delayer {
        ...
        static {
            (delayer = new ScheduledThreadPoolExecutor(1, new Delayer.DaemonThreadFactory())).setRemoveOnCancelPolicy(true);
        }
    }
    ...
    public static <V> CompletableFuture<V> timeoutAfter(long timeout, TimeUnit unit) {
        ...
        Delayer.delayer.schedule(() -> result.completeExceptionally(new TimeoutException("query time out")), timeout, unit);
        return result;
    }
     ...
     * @param <V>
     */
    public static <T, V> List<V> load(List<T> params, Function<T, V> function, Function<Pair<T, Throwable>, V> exceptionFunction,
                                      ExecutorService executor) {
        List<CompletableFuture<V>> listOfFutures = params.stream().map(param -> loadData(param, function, exceptionFunction, executor))
                .collect(toList());
     ...
     * @param <V>
     */
    public static <T, V> List<V> loadTimeout(List<T> params, Function<T, V> function, Function<Pair<T, Throwable>, V> exceptionFunction,
                                             ExecutorService executor, long timeout, TimeUnit unit) {

        List<CompletableFuture<V>> listOfFutures = params.stream().map(
                param -> loadDataTimeout(param, function, exceptionFunction, executor, timeout, unit))
                .collect(toList());
        CompletableFuture<List<V>> futureList = sequence(listOfFutures);
        return futureList.join();
    }
     ...
     * <p>loadData.</p>
     */
    public static <T, V> CompletableFuture<V> loadData(T param, Function<T, V> function, Function<Pair<T, Throwable>, V> exceptionFunction,
                                                       Executor executor) {
        return CompletableFuture.supplyAsync(() -> function.apply(param), executor).exceptionally(
                (throwable -> exceptionFunction.apply(new Pair<>(param, throwable))));
    }
    ...
    public static <T, V> CompletableFuture<V> loadDataTimeout(T param, Function<T, V> function,
                                                              Function<Pair<T, Throwable>, V> exceptionFunction,
                                                              Executor executor, long timeout, TimeUnit unit) {
        final CompletableFuture<V> timeoutFuture = timeoutAfter(timeout, unit);
        return CompletableFuture.supplyAsync(() -> function.apply(param), executor).applyToEither(timeoutFuture, Function.identity())
