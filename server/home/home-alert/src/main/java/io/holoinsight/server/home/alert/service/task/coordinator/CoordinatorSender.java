...
public class CoordinatorSender {
    ...
    public CoordinatorSender(List<String> otherMembers, String order, String periodId, CoordinatorService coordinatorService, OrderMap orderMap) {
    ...
    private void spreadGossip(String msg) {
        for (Map.Entry<String, ChannelFuture> entry : orderMap.channelMap.entrySet()) {
            ...
            channel.channel().writeAndFlush(Unpooled.wrappedBuffer(msg.getBytes())).addListener(new ChannelFutureListener() {
                @Override
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        LOGGER.info("spread gossip msg {} to {}", msg, ip);
                    } else {
                        LOGGER.info("fail to spread gossip msg {} to {} for {}", msg, ip, future.cause().getMessage());
                    }
                }
            });
        }
    ...
    private String buildPreOrderMsg(long heartbeat) {
        return String.join("__", "PREORDER", periodId, String.valueOf(heartbeat), ip, String.valueOf(order), "\r\n");
    }
}
