...
public class BiStreamService {
            ...
            stub.bistreamProxy(proxyRequest, new StreamObserver<BiStreamProxyResponse>() {
                ...
                public void onNext(BiStreamProxyResponse resp) {
                    if (bizType == 0 || resp.getType() == bizType + 1) {
                        ...
                        sink.error(new IllegalStateException("error bizType:" + bizType + " payload:" + resp.getPayload()));
                    }
                }
    ...
    public <RESP extends GeneratedMessageV3> void handleProxy(TargetIdentifier target, int bizType, GeneratedMessageV3 request, RESP defaultResp,
        StreamObserver<? super RESP> o) {
    ...
    public void proxy0(BiStreamProxyRequest request, Object defaultResp, StreamObserver<BiStreamProxyResponse> o) {
        ...
        if (s == null) {
            ...
            if (defaultResp instanceof GeneratedMessageV3) {
                respPayload = setRespHeader((GeneratedMessageV3) defaultResp, RpcCodes.RESOURCE_NOT_FOUND, msg).toByteString();
            } else {
        ...
        s.rpc(request.getBizType(), request.getPayload()) //
            ...
            .subscribe(respCmd -> {
                if (request.getBizType() == 0) {
                    BiStreamProxyResponse resp = BiStreamProxyResponse.newBuilder().setType(request.getBizType() + 1) //
                ...
                if (respCmd.getBizType() == request.getBizType() + 1) {
                    ...
                    if (respCmd.getBizType() == BizTypes.BIZ_ERROR) {
                        errorResp = setRespHeader(defaultResp0, RpcCodes.INTERNAL, respCmd.getData().toStringUtf8());
                    } else {
                        errorResp = setRespHeader(defaultResp0, RpcCodes.INTERNAL, "internal error: wrong bizType " + respCmd.getBizType());
                    }
                    BiStreamProxyResponse resp = BiStreamProxyResponse.newBuilder().setType(request.getBizType() + 1) //
                ...
                if (request.getBizType() == 0) {
                    ...
                    GeneratedMessageV3 errorResp = setRespHeader((GeneratedMessageV3) defaultResp, RpcCodes.INTERNAL, //
