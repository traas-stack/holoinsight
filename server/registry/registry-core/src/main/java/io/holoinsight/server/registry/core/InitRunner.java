...
public class InitRunner implements ApplicationRunner {
    ...
    private void startStat() {
        ...
        MetricsUtils.SM.gauge(StringsKey.of("apikey.count"), apikeyService, x -> x.getApikeyMap().size());

        Gauge templateCount = Gauge.build("registry_template_count", "registry_template_count").labelNames("tenant").register();
        Gauge agentCount = Gauge.build("registry_agent_count", "registry_template_count").labelNames("tenant").register();
        Gauge targetCount = Gauge.build("registry_target_count", "registry_target_count").labelNames("tenant").register();
        Gauge apikeyCount = Gauge.build("registry_apikey_count", "registry_apikey_count").register();

        Gauge agentConnectingCount = Gauge.build("registry_agent_connecting_count", "registry_agent_connecting_count") //
            .labelNames("tenant") //
            .register(); //

        commonThreadPools.getScheduler().scheduleWithFixedDelay(() -> {
            templateStorage.getTenantCountMap().forEach((tenant, count) -> templateCount.labels(tenant).set(count));
            agentStorage.getTenantCountMap().forEach((tenant, count) -> agentCount.labels(tenant).set(count));
            ...
            for (CollectTemplate t : templateStorage.readonlyLoop()) {
                ...
                if (count > 0) {
                    targetTenantCountMap.put(t.getTenant(), targetTenantCountMap.getOrDefault(t.getTenant(), 0) + count);
                }
            }
            ...
            {
                agentStorage.getConnectionTenantCountMap().forEach((tenant, count) -> agentConnectingCount.labels(tenant).set(count));
            }
