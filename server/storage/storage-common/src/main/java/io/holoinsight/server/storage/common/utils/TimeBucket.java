 ...
 */
package io.holoinsight.server.storage.common.utils;

import io.holoinsight.server.storage.common.exception.UnexpectedException;
...
public class TimeBucket {
  ...
  /**
   * The format of timeBucket in minute Unit is "yyyyMMddHHmmss", so which means the TimeBucket must
   * be between 10000000000000 and 99999999999999.
   */
  ...
  /**
   * The format of timeBucket in minute Unit is "yyyyMMddHHmm", so which means the TimeBucket must
   * be between 100000000000 and 999999999999.
   */
  ...
  /**
   * The format of timeBucket in hour Unit is "yyyyMMddHH", so which means the TimeBucket must be
  ...
  /**
   * The format of timeBucket in day Unit is "yyyyMMdd", so which means the TimeBucket must be
  ...
  public static long getTimeBucket(long timestamp, DownSampling downsampling) {
    ...
    switch (downsampling) {
      case Second:
        return year * 10000000000L + month * 100000000 + day * 1000000 + hour * 10000 + minute * 100
            + second;
      case Minute:
