...
public class AIAlarmLoadData implements AlarmLoadData {
    ...
    @Override
    public List<DataResult> queryDataResult(ComputeTask computeTask, InspectConfig inspectConfig, Trigger trigger) {
    ...
    private QueryProto.QueryResponse queryTags(ComputeTask computeTask, InspectConfig e, Trigger trigger) {
        ...
        try {
            ...
            for (DataSource dataSource : trigger.getDatasources()) {
                ...
                QueryProto.Datasource.Builder builder = QueryProto.Datasource.newBuilder()
                        ...
                        .setStart(start)
                        .setEnd(end)
                        .setMetric(dataSource.getMetric())
                        .addAllFilters(filterConvert(dataSource.getFilters()))
            ...
            request = QueryProto.QueryRequest.newBuilder()
                    .setTenant(e.getTenant())
                    .setQuery(trigger.getQuery())
                    .addAllDatasources(datasources)
                    .build();


            response = queryClientService.queryTag(request);
            LOGGER.info("QueryTags Success Request:{} Response:{}", G.get().toJson(request), G.get().toJson(response));
            return response;
        } catch (Exception exception) {
            LOGGER.error("QueryTags Exception Request:{} Response:{}", G.get().toJson(request), G.get().toJson(response), exception);
        }
        return null;
    }

    private QueryProto.QueryResponse queryDataCount(ComputeTask computeTask, InspectConfig e, Trigger trigger) {
        QueryProto.QueryResponse response = null;
        QueryProto.QueryRequest request = null;
        try {
            List<QueryProto.Datasource> datasources = new ArrayList<>();
            ...
            LOGGER.debug("QueryData Success Request:{} Response:{}", G.get().toJson(request), G.get().toJson(response));
            return response;
        } catch (Exception exception) {
            LOGGER.error("QueryData Exception Request:{} Response:{}", G.get().toJson(request), G.get().toJson(response), exception);
        }
        return null;
    }
