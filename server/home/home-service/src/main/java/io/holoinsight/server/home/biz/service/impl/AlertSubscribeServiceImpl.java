 ...
 */
@Service
public class AlertSubscribeServiceImpl extends ServiceImpl<AlarmSubscribeMapper, AlarmSubscribe> implements AlertSubscribeService {
    ...
    public Boolean saveDataBatch(AlarmSubscribeDTO alarmSubscribeDTO, String creator, String tenant) {
        ...
        List<Long> ids = alarmSubscribes.stream().map(AlarmSubscribe::getId).collect(Collectors.toList());
        List<AlarmSubscribeInfo> alarmSubscribeInfo = alarmSubscribeDTO.getAlarmSubscribe();
        List<Long> updateIds = alarmSubscribeInfo.stream().map(AlarmSubscribeInfo::getId).collect(Collectors.toList());
