...
public class NotificationTemplate {
  ...
  private String getValue(TemplateValue templateValue, AlertTemplateField field,
    ...
    switch (field) {
      ...
      case ALERT_METRIC:
      case metric:
        return templateValue.getMetric();
      case ALERT_PRIORITY:
      case alarmLevel:
        if (templateValue.getAlarmLevel() == null) {
          return StringUtils.EMPTY;
        }
        return templateValue.getAlarmLevel().getDesc();
      case ALERT_QUERY:
        return templateValue.getAlertQuery();
      case ALERT_SCOPE:
      case alarmTags:
        return buildTagValue(templateValue.getAlarmTags(), tagMarkdown);
      case ALERT_STATUS:
        return buildSummary();
      case ALERT_DURATION:
        return String.valueOf(templateValue.getDuration());
      case ALERT_VALUE:
      ...
      case ALERT_TYPE:
        return templateValue.getRuleConfig().getRuleType();
      case DATE:
      case alarmTime:
        return templateValue.getAlarmTime();
      case ALERT_START_TIME:
        return calculateStartTime(templateValue);
      case alertTimestamp:
      case alarmTimestamp:
        return templateValue.getAlarmTimestamp();
      case EVENT_MSG:
      case alarmContent:
        return templateValue.getAlarmContent();
      case EVENT_TYPE:
        return templateValue.getMetricType();
      case SOURCE_TYPE:
        return templateValue.getSourceType();
      case HOSTNAME:
        return templateValue.getAlertServer();
      case IP:
      ...
      case ALERT_TRIGGER_CONDITION:
        return templateValue.getTriggerCondition();
      case LOG_CONTENT:
  ...
  private String calculateStartTime(TemplateValue templateValue) {
    if ((StringUtils.isBlank(templateValue.getAlarmTimestamp())
        || !StringUtils.isNumeric(templateValue.getAlarmTimestamp()))
        || templateValue.getDuration() == null) {
      return StringUtils.EMPTY;
    }
    long timestamp = Long.parseLong(templateValue.getAlarmTimestamp());
    long duration = templateValue.getDuration() * 60000L;
