...
@Component
public class RegistryServiceForAgentImpl extends RegistryServiceForAgentGrpc.RegistryServiceForAgentImplBase {
    private static final Logger                      LOGGER                    = LoggerFactory.getLogger(RegistryServiceForAgentImpl.class);
    private static final StatAccumulator<StringsKey> GRPC_AGENT_HEARTBEAT_STAT = MetricsUtils.SM1S.create("grpc.agent.heartbeat");
    private static final StatAccumulator<StringsKey> AGENT_PULLCONFIG_STAT     = MetricsUtils.SM.create("agent.pullconfig.stat");
    ...
    @Override
    public void sendAgentHeartbeat(SendAgentHeartbeatRequest request, StreamObserver<SendAgentHeartbeatResponse> o) {
    ...
    @Override
    public void getCollectTasks(GetCollectTasksRequest request, StreamObserver<GetCollectTasksResponse> o) {
        maybeEnableCompression(o);
        ...
        apikeyAuthService.get(header.getApikey()) //
            ...
            .subscribe(o::onNext, error -> {
                ...
                if (error instanceof AuthErrorException) {
                    o.onError(Status.UNAUTHENTICATED.withDescription(error.getMessage()).asRuntimeException());
                } else {
    ...
    private GetCollectTasksResponse getCollectTasks0(GetCollectTasksRequest request) {
        ...
        if (agent == null) {
            throw new StatusRuntimeException(Status.UNAUTHENTICATED.withDescription("no agent with id " + request.getAgentId()));
        }
        ...
        keys.sort(Comparator.comparingLong(CollectTargetKey::getTemplateId).thenComparing(CollectTargetKey::getDimId));
            ...
            for (CollectTargetKey key : keys) {
                ...
                if (processedDims.add(key.getDimId())) {
                    ...
                    if (key.getDimId().startsWith("dim2:")) {
                        Map<String, Object> dim = prodDimService.queryByDimId(t.getCollectRange()
                                .getCloudmonitor()
                                .getTable(),
        ...
        LOGGER.info("agent={} keys={} missDim={} tasks={}", request.getAgentId(), keys.size(), missDim, count);
