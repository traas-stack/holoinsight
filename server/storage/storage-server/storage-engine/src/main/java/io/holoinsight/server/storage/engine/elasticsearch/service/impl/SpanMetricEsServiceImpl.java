...
public class SpanMetricEsServiceImpl implements MetricEsService {
    ...
    public MetricValues queryMetric(String tenant, String metric, Duration duration, Map<String, Object> conditions)
        ...
        return queryFromEs(tenant, metricDefine.getIndex(), duration, mergedConditions, metricDefine.getField(),
    ...
    public List<String> querySchema(String metric) {
        ...
        MetricDefine metricDefine = metricsManager.getMetric(metric);
        Assert.notNull(metricDefine, String.format("metric not found: %s", metric));
        return metricDefine.getGroups().stream().map(OtlpMappings::otlp2Sw).collect(Collectors.toList());
    }
    ...
    private MetricValues queryFromEs(String tenant, String index, Duration duration, Map<String, Object> conditions,
        ...
        boolQueryBuilder.filter(new TermQueryBuilder(SpanEsDO.resource(SpanEsDO.TENANT), tenant)).filter(
                new RangeQueryBuilder(SpanEsDO.START_TIME).gte(duration.getStart()).lte(duration.getEnd()));
        ...
        DateHistogramAggregationBuilder dateHistogramAggregationBuilder = AggregationBuilders
                .dateHistogram(SpanEsDO.START_TIME)
                .field(SpanEsDO.START_TIME)
